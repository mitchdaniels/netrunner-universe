...
jQuery.getJSON(GetQueryStringParams("config","configRunner.json"), function(data, textStatus, jqXHR) {
	config=data;
...
function nodeActive(a) {
    ...
    if (f.attributes) {
        ...
        if (image_attribute) {
        	...
        	$GP.info_name.html("<div><img src=" + f.attributes[image_attribute] + " style=\"vertical-align:middle\" /> <span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' /*+ b.attr.attributes.title*/ + "</span></div>");
        } else {
        	$GP.info_name.html("<div><span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' + b.attr.attributes.title + "</span></div>");
        }
